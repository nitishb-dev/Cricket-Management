...
import { Player, Match, PlayerStats, MatchPlayerStats } from '../types/cricket'
...
export const CricketProvider: React.FC<CricketProviderProps> = ({ children }) => {
  ...
  const addPlayer = async (name: string): Promise<Player | null> => {
    try {
      ...
      // Refresh the entire players list to ensure we have the latest data
      await refreshData()
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/matches`, {
        ...
        body: JSON.stringify({
          ...
          teamAScore: matchData.teamA.players.reduce((sum: number, p: any) => sum + p.runs, 0),
          teamAWickets: matchData.teamA.players.reduce((sum: number, p: any) => sum + p.wickets, 0),
          teamBScore: matchData.teamB.players.reduce((sum: number, p: any) => sum + p.runs, 0),
          teamBWickets: matchData.teamB.players.reduce((sum: number, p: any) => sum + p.wickets, 0),
          winner: matchData.winner,
          manOfMatch: matchData.manOfMatch,
