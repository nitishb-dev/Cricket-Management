      ...
      <div className="space-y-2 max-h-[400px] overflow-y-auto">
        {loading ? (
          ...
          filteredPlayers.map(player => {
            ...
            if (isEditing) {
              return (
                <div key={player.id} className="flex items-center justify-between p-3 sm:p-4 rounded-lg border-2 border-green-500 bg-green-50">
                  <input
                    type="text"
                    value={editedName}
                    ...
                    autoFocus
                    className="font-medium text-sm sm:text-base bg-transparent focus:outline-none w-full"
                  />
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {isUpdating ? <div className="animate-spin rounded-full h-4 w-4 border-2 border-green-500 border-t-transparent"></div> : <Save size={18} />}
                    </button>
                    <button onClick={() => setEditingPlayerId(null)} className="text-red-500 hover:text-red-700 transition">
                      <XCircle size={18} />
                    </button>
                  </div>
                </div>
