...
export const MatchPlay: React.FC<MatchPlayProps> = ({
  ...
  useEffect(() => {
    ...
    if (selectedBatsman && !players.find(p => p.id === selectedBatsman.player.id)) {
      setSelectedBatsman(null);
    }
    // If selected bowler is not in the current players list, deselect them
    if (selectedBowler && !players.find(p => p.id === selectedBowler.player.id)) {
