...
// type StatColor = 'yellow' | 'red' | 'green' | 'purple'
// const colorMap: Record<StatColor, { from: string; to: string; text: string }> = {
...
//       console.log("DEBUG: fetched playerStats:", stats)
...
//   const safeTop = (key: keyof CricketPlayerStats): CricketPlayerStats | undefined =>
//     playerStats.length > 0
//       ? playerStats.reduce((prev, curr) =>
//           (curr[key] ?? 0) > (prev[key] ?? 0) ? curr : prev
//         )
//       : undefined

//   const topPerformers = {
//     runs:    safeTop('totalRuns'),
//     wickets: safeTop('totalWickets'),
//     matches: safeTop('totalMatches'),
//     mom:     safeTop('manOfMatchCount'),
//   }
...
//             {[
//               { title: 'Top Run Scorer', value: topPerformers.runs?.totalRuns, name: topPerformers.runs?.player.name, color: 'yellow', icon: Target },
//               { title: 'Top Wicket Taker', value: topPerformers.wickets?.totalWickets, name: topPerformers.wickets?.player.name, color: 'red', icon: TrendingUp },
//               { title: 'Most Matches', value: topPerformers.matches?.totalMatches, name: topPerformers.matches?.player.name, color: 'green', icon: Trophy },
//               { title: 'Most MoM Awards', value: topPerformers.mom?.manOfMatchCount, name: topPerformers.mom?.player.name, color: 'purple', icon: Award }
//             ].map(({ title, value, name, color, icon: Icon }, idx) => {
//               const { from, to, text } = colorMap[color as StatColor]
//               return (
...
//                       <p className={`${text} text-sm`}>{name ?? 'N/A'}</p>
//                     </div>
...
//                       <th key={i} className={`px-6 py-4 text-sm font-semibold text-gray-600 ${h === 'Player' ? 'text-left': 'text-center'}`}>
//                         {h}
//                       </th>
//                     ))}
//                   </tr>
...
//                             Joined {new Date(s.player.created_at).toLocaleDateString()}
//                           </div>
//                         </div>
//                       </td>
