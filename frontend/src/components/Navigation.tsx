  ...
  return (
    <>
      ...
      <nav className="md:hidden bg-white shadow-lg">
        <div className="flex items-center justify-center py-4 border-b-4 border-green-600">
          <Trophy className="text-green-600 mr-2" size={28} />
          ...
          {navItems.map(item => {
            ...
            return (
              <button
                key={item.id}
                onClick={() => onNavigate(item.id)}
                className={`flex flex-col items-center gap-1 px-4 py-3 min-w-0 flex-shrink-0 transition-all duration-200 ${
                  isActive
